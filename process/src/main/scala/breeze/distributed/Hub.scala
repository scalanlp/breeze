...
object Hub {
  ...
  /** Connects to the hub at the URI specified in the breeze.distributed.hub system property. */
  def connect() = {
    val uri = System.getProperty("breeze.distributed.hub");
    if (uri == null) {
      throw new IllegalArgumentException("System property breeze.distributed.hub not defined.")
    }

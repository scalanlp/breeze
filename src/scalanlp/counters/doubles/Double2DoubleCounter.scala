...
trait Double2DoubleCounter extends DoubleCounter[Double] {
  ...
  override  def topK(k : Int) = Counters.topK[(Double,Double)](k,(x,y) => (x._2-y._2).asInstanceOf[Int])(this);
  ...
  override  def +=(that : Iterable[(Double,Double)]) {
    for(val (k,v) <- that) {
...
object Double2DoubleCounter {
  ...
  def apply() = new MapWrapper[Double,Double] with Double2DoubleCounter {

...
trait Double2ShortCounter extends ShortCounter[Double] {
  ...
  override  def topK(k : Int) = Counters.topK[(Double,Short)](k,(x,y) => (x._2-y._2).asInstanceOf[Int])(this);
  ...
  override  def +=(that : Iterable[(Double,Short)]) {
    for(val (k,v) <- that) {
...
object Double2ShortCounter {
  ...
  def apply() = new MapWrapper[Double,Short] with Double2ShortCounter {

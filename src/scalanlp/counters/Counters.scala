...
object FastUtil {
  ...
  @inline implicit def iObject2IntFastEntrySet[K](jcol : Object2IntMap.FastEntrySet[K]) : Iterable[(K,Int)] = {
    new Iterable[(K,Int)] {
      def elements = iObject2IntFastIterator[K](jcol.fastIterator())
    }
  }
  ...
  @inline implicit def iObject2DoubleFastEntrySet[K](jcol : Object2DoubleMap.FastEntrySet[K]) : Iterable[(K,Double)] = {
    new Iterable[(K,Double)] {
      def elements = iObject2DoubleFastIterator[K](jcol.fastIterator())
    }
  }
}
...
object CounterScratchSpace {
  abstract trait AbstractCounter[K,V] extends scala.collection.mutable.Map[K,V] {
    /** Value "1" for incrementing/decrementing in this map */
    def one : V
      
    /** Value "0" for acting as default value */
    def zero : V
      
    /** Function to add two values together.  May re-use and return a. */
    def valueAdd (a : V, b : V) : V
      
    /** Function to subtract one value from another.  May re-use and return a. */
    def valueSub (a : V, b : V) : V
      
    /** Increments the given key by this.one */
    def increment(key : K) : Unit = increment(key, one)
      
    /** Decrements the given key by this.one */
    def decrement(key : K) : Unit = decrement(key, one)
      
    /** Increments the given key by the given amount */
    def increment(key : K, value : V) : Unit = {
      put(key, valueAdd(this.getOrElse(key,zero),value))
    }
      
    /** Decrements the given key by the given amount */
    def decrement(key : K, value : V) : Unit = {
      put(key, valueSub(this.getOrElse(key,zero),value))
    }
  }
    
  trait MyIntCounter[K] extends AbstractCounter[K,Int] {
    def zero = 0
    def one  = 1
    def valueAdd (a : Int, b : Int) = a + b
    def valueSub (a : Int, b : Int) = a - b
  }
    
  def test() : Unit = {
    val counter = new scala.collection.mutable.HashMap[String,Int] with MyIntCounter[String];
      
    val v : AnyVal = 3
    v match {
      case vi : Int => println("int!")
    }

...
trait Float2ShortCounter extends ShortCounter[Float] {
  ...
  override  def topK(k : Int) = Counters.topK[(Float,Short)](k,(x,y) => (x._2-y._2).asInstanceOf[Int])(this);
  ...
  override  def +=(that : Iterable[(Float,Short)]) {
    for(val (k,v) <- that) {
...
object Float2ShortCounter {
  ...
  def apply() = new MapWrapper[Float,Short] with Float2ShortCounter {

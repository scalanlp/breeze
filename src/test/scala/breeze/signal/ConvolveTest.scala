...
class ConvolveTest extends FunSuite {
  ...
  test("convolve/correlate") {
    ...
    assert( correlate(data, kernel, overhang = OptOverhang.PreserveLength ) == DenseVector(8.0, 11.0, 14.0, 5.0) )

    assert( convolve(data, kernel, 0 to 1 ) == DenseVector(7.0, 10.0) )
    assert( correlate(data, kernel, 2 to -1, overhang = OptOverhang.Full ) == DenseVector( 11.0, 14.0, 5.0 ) )

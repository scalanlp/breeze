 ...
 * @author dlwh
 */
class StaticHTMap[+T] (private val map: Map[String,Any]) {
  ...
  def +[U](x: U)(implicit m: Manifest[U]) = new StaticHTMap[T with U](map + (m.toString->x))
   ...
   */
  def -+[U>:T](x:U)(implicit m: Manifest[U]) = new StaticHTMap[T] (map + (m.toString->x));
  
  /**
   * Applys f to this and combines the result with this map.
   */
  def and[U](f: StaticHTMap[T]=>U)(implicit m: Manifest[U]): StaticHTMap[T with U] = {
    val newPart = f(this); 
    this.+[U](newPart);
  }
  
  /** Concatenates two maps. values in "other" override values in "this" */
  def ++[U](other: StaticHTMap[U]): StaticHTMap[T with U] = {
    new StaticHTMap[T with U](this.map ++ other.map);
  }
...
object StaticHTMap {
   ...
   * at run time.
   */
  def apply() = new StaticHTMap[Any](Map.empty);
  
  /**
   * Construct an HTMap with one initial value
   */
  def apply[T](x:T)(implicit m: Manifest[T]) = new StaticHTMap[T](Map.empty + (m.toString->x))
  
  /**
   * Construct an HTMap with two initial values
   */
  def apply[T,T2](x:T, x2:T2)(implicit m: Manifest[T], m2: Manifest[T2]) = {
    new StaticHTMap[T with T2](Map.empty ++ List( (m.toString->x) , (m2.toString->x2)))
  }
  ...
  def apply[T,T2,T3](x:T, x2:T2, x3: T3)(implicit m: Manifest[T], m2: Manifest[T2], m3: Manifest[T3]) = {
    new StaticHTMap[T with T2 with T3](Map.empty ++ List( (m.toString->x) , (m2.toString->x2), (m3.toString->x3)))
  }
}

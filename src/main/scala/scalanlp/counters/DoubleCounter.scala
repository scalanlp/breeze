...
trait DoubleCounter[T] extends Map[T,Double] {
  ...
  */
  ...
  override def default(k : T) : Double = defaultValue;
  ...
  def -=(that : DoubleCounter[T]) {
   ...
   def unary_-() = {
      val rv : DoubleCounter[T] = DoubleCounter[T]();
      rv -= this;
      rv;
  }
}
...
object DoubleCounter {
  ...
  @serializable
  @SerialVersionUID(2L)
  class FastMapCounter[T](override val defaultValue: Double) extends MapWrapper[T,Double] with DoubleCounter[T] {
  ...
  def withDefaultValue[T](d: Double) = new FastMapCounter[T](d);
  ...
  def apply[T](implicit m : scala.reflect.Manifest[T]) : DoubleCounter[T] = fromClass(m.erasure.asInstanceOf[Class[T]]);
    
  def fromClass[T](c: Class[T]) : DoubleCounter[T] = c match {

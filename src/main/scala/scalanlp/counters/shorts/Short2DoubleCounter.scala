...
/**
 * Count objects of type Short with type Double.
...
trait Short2DoubleCounter extends DoubleCounter[Short] {
   ...
   */
   override  def incrementCount(t : Short, v : Double) = {
     update(t,(this(t) + v).asInstanceOf[Double]);
   }


  override def ++=(kv: Iterable[(Short,Double)]) = kv.foreach(+=);

  /**
   * Increments the count associated with Short by Double.
   * Note that this is different from the default Map behavior.
  */
  override def +=(kv: (Short,Double)) = incrementCount(kv._1,kv._2);

  override def default(k : Short) : Double = defaultValue;

  override  protected def defaultValue: Double = 0;
  ...
  /**
   * Return a new Short2DoubleCounter with each Double divided by the total;
   */
   ...
   */
  override  def topK(k : Int) = Counters.topK[(Short,Double)](k,(x,y) => if(x._2 < y._2) -1 else if (x._2 == y._2) 0 else 1)(this);
  ...
  override  def unary_-() = {
      val rv : DoubleCounter[Short] = Short2DoubleCounter();
      rv -= this;
      rv;
  }
}
...
object Short2DoubleCounter {
  ...
  @serializable
  @SerialVersionUID(2L)
  class FastMapCounter(override val defaultValue: Double) extends MapWrapper[Short,Double] with Short2DoubleCounter {

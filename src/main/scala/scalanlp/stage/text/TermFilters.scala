...
import scalanlp.counters.IntCounter;
...
case object TermCounter extends MetaBuilder[TermCounts,Batch[Seq[String]]] {
  override def build(data : Batch[Seq[String]]) =
    new TermCounts(data.values.elements);
}
...
extends Mapper1[Seq[String],Seq[String],TermCounts] {
  override def map(freq : TermCounts)(doc : Seq[String]) = {
    for (term <- doc; if freq.docCounts(term) >= minDF) yield term;
  }
}
...
extends Stage[Any,TermStopListFilter,Batch[Seq[String]],Batch[Seq[String]]] {
  override def process[M]
  (parcel : Parcel[M,Batch[Seq[String]]])
  : Parcel[M with TermStopListFilter, Batch[Seq[String]]] = {
    parcel.withHistory(this).withMeta(this).map(_.map(_.filter(term => !stops.contains(term))));
  }
}
...
extends Stage[TermCounts,TermStopListFilter,Batch[Seq[String]],Batch[Seq[String]]] {
  override def process[M<:TermCounts]
  (parcel : Parcel[M,Batch[Seq[String]]])
  : Parcel[M with TermStopListFilter, Batch[Seq[String]]] = {
    val termCounts = parcel.meta[TermCounts].termCounts;
    val stops = Set() ++ termCounts.topK(numTerms).map(_._1);
    val stop = TermStopListFilter(stops);
    parcel.withHistory(this).withMeta(stop).map(_.map(_.filter(term => !stops.contains(term))));
  }
}

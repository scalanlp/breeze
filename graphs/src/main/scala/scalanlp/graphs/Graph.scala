...
object Graph {
   ...
   */
  def fromEdges[N](edges: (N,N)*):Graph{ type Node = N; type Edge = (N,N);} = Digraph.fromEdges(edges:_*);
  /**
   * Constructs a graph from an adjacency list. The list must be symmetric.
   */
  def fromAdjacencyList[N](adjacencyList: Map[N,Seq[N]]):Graph{ type Node = N; type Edge = (N,N);} = {
...
object Digraph {
   ...
   */
  def fromAdjacencyList[N](adjacencyList: Map[N,Seq[N]]) = {

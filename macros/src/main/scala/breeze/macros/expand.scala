...
object expand {
    ...
    annottees.head.tree match {
      case tree @ DefDef(mods, name, targs, vargs, tpt, rhs) =>
        ...
        val newDefs = configurations.map { typeMap =>
          ...
          val newvargs =
            valsToLeave.filterNot(_.isEmpty).map(_.map(substitute(c)(typeMap, valExpansions, _).asInstanceOf[ValDef]))
  ...
  def substitute(c: Context)(
    ...
    val termTypeMap = typeMap.map {
      case (name, tpe) => (name.toTermName: c.Name) -> Ident(tpe.typeSymbol.name.toTermName)
    }

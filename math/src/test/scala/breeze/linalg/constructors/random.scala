...
class randomTest extends FunSuite {
  ...
  test("randomDouble/randomInt"){
    ...
    val maxRD1000 = max(randDoub1000)
    val minRD1000 = min(randDoub1000)
    assert( maxRD1000 > 14.5 && maxRD1000 <= 15 && minRD1000 < 0.5 && minRD1000 >= 0)

    val randI1000 = randomInt( (17, 202), (5, 9) )
    val maxRI1000 = max(randI1000)
    val minRI1000 = min(randI1000)
    assert(  maxRI1000 > 8 && maxRI1000 <= 9 && minRI1000 < 1 && minRI1000 >= 0 )

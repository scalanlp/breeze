...
object MarkovChain {
  ...
  object Combinators {
    ...
    class RichKernel[T](k1: T => Rand[T]) {
       ...
       */
      def promoteSeq: Seq[T] => Rand[Seq[T]] = { (t: Seq[T]) =>
        promote(t.map(k1));
      }
       ...
       */
      def promoteIterable: Iterable[T] => Rand[Iterable[T]] = { (t: Iterable[T]) =>
        promote(t.map(k1));
      }
     ...
     */
    def promoteTuple[T1, T2, T3, U1, U2, U3](
        k1: (T1, T2, T3) => Rand[U1],
        k2: (U1, T2, T3) => Rand[U2],
     ...
     * Tupleization of nearly-transition kernels to produce a kernel for tuples
     */
    def promoteTuple[T1, T2, T3, U1, U2, U3, T4, U4](
        k1: (T1, T2, T3, T4) => Rand[U1],
  ...
  object Kernels {
     ...
     *
     */
    def metropolis[T](proposal: T => Rand[T])(logMeasure: T => Double)(implicit rand: RandBasis = Rand) = { t: T =>
      for (next <- proposal(t);
     ...
     *
     */
    def metropolisHastings[T](proposal: T => ContinuousDistr[T])(logMeasure: T => Double)(

...
trait DenseMatrixMultiplyStuff extends DenseMatrixOps
  ...
  implicit object  implPowMatrixMatrix_DMD_D_eq_DMC    extends  OpPowerMatrix.Impl2[DenseMatrix[Double], Double, DenseMatrix[Complex]] {
      def apply(A : DenseMatrix[Double], p : Double): DenseMatrix[Complex] = {
      require(A.rows == A.cols, "Non-square matrix")
        matrixPow(A, p)
     }
  }

    implicit object  implPowMatrixMatrix_DMC_D_eq_DMC    extends  OpPowerMatrix.Impl2[DenseMatrix[Complex], Double, DenseMatrix[Complex]] {
      def apply(A : DenseMatrix[Complex], p : Double): DenseMatrix[Complex] = {
      require(A.rows == A.cols, "Non-square matrix")
        matrixPow(A, p)
     }
  }

     implicit object  implPowMatrixMatrix_DMI_D_eq_DMC    extends  OpPowerMatrix.Impl2[DenseMatrix[Int], Double, DenseMatrix[Complex]] {
      def apply(A : DenseMatrix[Int], p : Double): DenseMatrix[Complex] = {
      require(A.rows == A.cols, "Non-square matrix")
        matrixPow(A, p)
     }
  }

      implicit object  implPowMatrixMatrix_DMD_I_eq_DMC    extends  OpPowerMatrix.Impl2[DenseMatrix[Double], Int, DenseMatrix[Complex]] {
      def apply(A : DenseMatrix[Double], p : Int): DenseMatrix[Complex] = {
      require(A.rows == A.cols, "Non-square matrix")
        matrixPow(A, p)
     }
  }

        implicit object  implPowMatrixMatrix_DMC_I_eq_DMC    extends  OpPowerMatrix.Impl2[DenseMatrix[Complex], Int, DenseMatrix[Complex]] {
      def apply(A : DenseMatrix[Complex], p : Int): DenseMatrix[Complex] = {
      require(A.rows == A.cols, "Non-square matrix")
        matrixPow(A, p)
     }
  }

          implicit object  implPowMatrixMatrix_DMI_I_eq_DMC    extends  OpPowerMatrix.Impl2[DenseMatrix[Int], Int, DenseMatrix[Complex]] {
      def apply(A : DenseMatrix[Int], p : Int): DenseMatrix[Complex] = {
      require(A.rows == A.cols, "Non-square matrix")
        matrixPow(A, p)
     }
  }

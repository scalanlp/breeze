...
private object SliceUtils {
  def mapColumnSeq[V](cols: Seq[Int], nCols: Int) =
    cols match {
  ...
  def mapColumn[V](col: Int, nCols: Int): Int =
    col match {
  ...
  def mapRowSeq[V](rows: Seq[Int], nRows: Int): IndexedSeq[Int] =
    rows match {
  ...
  def mapRow[V](row: Int, nRows: Int): Int =
    row match {

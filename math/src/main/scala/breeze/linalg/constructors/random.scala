...
trait RandomGeneratorUFunc[T] extends UFunc {
  ...
  implicit def implRandomT_1D(implicit basis: RandBasis = Rand): Impl[Int, DenseVector[T]] =
    new Impl[Int, DenseVector[T]] {
      def apply(dimensions1: Int): DenseVector[T] = DenseVector.rand(dimensions1, gen)
    }

  implicit def implRandomT_1DRange(implicit basis: RandBasis = Rand): Impl2[Int, (T, T), DenseVector[T]] =
  ...
  implicit def implRandomT_2D(implicit basis: RandBasis = Rand): Impl[(Int, Int), DenseMatrix[T]] =
  ...
  implicit def implRandomT_2DRange(implicit basis: RandBasis = Rand): Impl2[(Int, Int), (T, T), DenseMatrix[T]] =

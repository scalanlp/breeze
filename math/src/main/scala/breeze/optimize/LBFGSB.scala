...
class LbfgsbSolver(maxIter:Int, tolerance:Double)
  ...
  override protected def chooseDescentDirection(state: State, f: DiffFunction[DenseVector[Double]]): DenseVector[Double] = {
    ...
    if (!isConvergence(x,g)) {
      throw new FirstOrderException("The steepest descent direction on to the feasible region can not be converge.\n" +
        "norm(P(xk-gk, l, u) - xk, INF) < 1E-5 test failed!")
    }
  ...
  override protected def takeStep(state: State, dir: DenseVector[Double], stepSize: Double) = state.x + dir * stepSize

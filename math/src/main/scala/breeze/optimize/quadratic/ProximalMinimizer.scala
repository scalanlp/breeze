...
class ProximalMinimizer(nGram: Int,
  ...
  def setLambda(lambda: Double): ProximalMinimizer = {
    this.lambda = lambda
    this
  }
  ...
  def setProximal(constraint: Constraint): ProximalMinimizer = {
  ...
  def updateGram(row: Int, col: Int, value: Double) {
    if (row < 0 || row >= n) {
      throw new IllegalArgumentException("ProximalMinimizer row out of bounds for gram matrix update")
    }
    if (col < 0 || col >= n) {
      throw new IllegalArgumentException("ProximalMinimizer column out of bounds for gram matrix update")
    }
  ...
  def solve(q: DoubleMatrix): (DoubleMatrix, Boolean) = {
    ...
    if (linearEquality > 0 && beq.get.rows != linearEquality) {
      throw new IllegalArgumentException("ProximalMinimizer beq vector should match the number of linear equalities")
    }
    ...
    while (k < MAX_ITER) {
      ...
      constraint match {
        ...
        case BOUNDS => {
          if (lb == None && ub == None)
            throw new IllegalArgumentException("ProximalMinimizer proximal operator on box needs lower and upper bounds")
          Proximal.projectBox(z.data, lb.get.data, ub.get.data)
        }
        case EQUALITY => {
          if (Aeq == None) throw new IllegalArgumentException("ProximalMinimizer proximal operator on equality needs Aeq")
          if (beq == None) throw new IllegalArgumentException("ProximalMinimizer proximal operator on equality needs beq")
...
object ProximalMinimizer {
  def apply(rank: Int, constraint: Constraint, lambda: Double): ProximalMinimizer = {
    constraint match {
      case SMOOTH => new ProximalMinimizer(rank)
      case POSITIVE => new ProximalMinimizer(rank).setProximal(POSITIVE)
      case BOUNDS => {
        //Direct QP with bounds
        val lb = DoubleMatrix.zeros(rank, 1)
        val ub = DoubleMatrix.zeros(rank, 1).addi(1.0)
        new ProximalMinimizer(rank, Some(lb), Some(ub)).setProximal(BOUNDS)
      }
      case EQUALITY => {
        //Direct QP with equality and positivity constraint
        val Aeq = DoubleMatrix.ones(1, rank)
        val beq = DoubleMatrix.ones(1, 1)
        val qm = new ProximalMinimizer(rank, None, None, Some(Aeq), Some(beq), true).setProximal(POSITIVE)
        qm
      }
      case SPARSE => {
        val qm = new ProximalMinimizer(rank).setProximal(SPARSE)
        qm.setLambda(lambda)
        qm
      }
    }
  ...
  def main(args: Array[String]) {
    ...
    println(s"Test ProximalMinimizer, breeze LBFGS and jblas solvePositive with n $problemSize m $nequalities")
    ...
    val qpSolver = new ProximalMinimizer(problemSize)
    ...
    val qpSolverPos = ProximalMinimizer(n, POSITIVE, 0.0)
    ...
    val qpSolverBounds = new ProximalMinimizer(n, Some(lb), Some(ub), None).setProximal(BOUNDS)
    ...
    val qpSolverL1 = new ProximalMinimizer(problemSize, None, None, None).setProximal(SPARSE)
    ...
    val directQpMl = new ProximalMinimizer(ml, Some(lbeq), Some(ubeq), Some(Aeq), Some(beq), true)
    ...
    val qpSolverMlL1 = ProximalMinimizer(25, SPARSE, 2.0)
    ...
    val qpIters = ProximalMinimizer(20, POSITIVE, 0.0)
    ...
    println(s"ProximalMinimizer iters ${qpIters.iterations} result ${posResult2.toString()}")

    val ws1 = NNLS.createWorkspace(25)
    val qpIters2 = ProximalMinimizer(25, POSITIVE, 0.0)
    
    println("NNLS test2")
    
    val nnlsResult3 = NNLS.solve(P3, q3.mul(-1), ws1)
    println(s"NNLS iters ${ws.iterations} result ${nnlsResult2.toList.mkString(",")}")
    
    val (posResult3, posConverged3) = qpIters2.solve(P3, q3)
    println(s"ProximalMinimizer iters ${qpIters2.iterations} result ${posResult3.toString()}")
    ...
    val qpSparse = ProximalMinimizer(20, SPARSE, 0.06435)
    ...
    val sparseQp = ProximalMinimizer(h.rows, SPARSE, lambdaL1)
    ...
    val posQp = new ProximalMinimizer(h.rows).setProximal(POSITIVE)
    ...
    val boundsQp = new ProximalMinimizer(h.rows, Some(bl), Some(bu)).setProximal(BOUNDS)
    ...
    val qpEquality = new ProximalMinimizer(h.rows, None, None, Some(Aeq), Some(beq), true).setProximal(POSITIVE)

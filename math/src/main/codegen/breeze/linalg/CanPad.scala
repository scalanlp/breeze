  ...
  implicit def implDM_OptPadDim_OptPadMode[@expand.args(Int, Long, Float, Double) T: ClassTag: Semiring]
    ...
    new CanPadRight[DenseMatrix[T], Dimensions2, DenseMatrix[T]] {
      ...
      def padRight2ImplFixed(m: DenseMatrix[T], optDim: Dimensions2, padValue: T): DenseMatrix[T] = {
        ...
        cforRange2(0 until min(optDim.n2, m.cols), 0 until min(optDim.n1, m.rows)) { (c, r) =>
          tempret(r, c) = m(r, c)
        }
        tempret
      }
...
object CanPadLeft {
  ...
  implicit def implDV_OptPadDim[@expand.args(Int, Long, Float, Double) T: ClassTag: Semiring]
    ...
    new CanPadLeft[DenseVector[T], Dimensions1, DenseVector[T]] {
      ...
      def padLeft1ImplZero(v: DenseVector[T], optDim: Dimensions1): DenseVector[T] = {
      ...
      def padLeft1ImplDV(v: DenseVector[T], optDim: Dimensions1, padDV: DenseVector[T]): DenseVector[T] = {
        ...
        v.length match {
          case optDim.n1 => v.copy
          case num: Int if num < optDim.n1 =>
            DenseVector(reverse(reverse(padDV).apply(0 until optDim.n1 - num)).toArray ++ v.toArray)
          case num: Int if optDim.n1 < num => v(0 until optDim.n1).copy //function should return a copy
          case _ => throw new IllegalArgumentException("(n) specification incorrect: " + optDim.toString + " !")
        }
      }
  ...
  implicit def implDM_OptPadDim_OptPadMode[@expand.args(Int, Long, Float, Double) T: ClassTag: Semiring]
    ...
    new CanPadLeft[DenseMatrix[T], Dimensions2, DenseMatrix[T]] {
      ...
      def padLeft2ImplFixed(m: DenseMatrix[T], optDim: Dimensions2, padValue: T): DenseMatrix[T] = {
        ...
        val tempret = DenseMatrix.zeros[T](optDim.n1, optDim.n2)
        cforRange2(1 to min(optDim.n2, m.cols), 1 to min(optDim.n1, m.rows)) { (c, r) =>

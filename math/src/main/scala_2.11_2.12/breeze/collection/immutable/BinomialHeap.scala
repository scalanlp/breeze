...
class BinomialHeap[T]()(implicit ord: Ordering[T])
  ...
  private def merge(l1: List[Node[T]], l2: List[Node[T]], acc: List[Node[T]]): List[Node[T]] =
    (l1, l2) match {
...
object BinomialHeap {
  ...
  def empty[T: Ordering]: BinomialHeap[T] =
    new BinomialHeap[T] {
      override val trees = Nil
    }

  private def mkHeap[T: Ordering](ns: List[Node[T]], sz: Int) =
    new BinomialHeap[T] {
